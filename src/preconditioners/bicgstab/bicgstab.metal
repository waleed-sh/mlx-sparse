...
template <typename I>
[[kernel]] void csr_bicgstab_jacobi_kernel(
  ...
  device float *p_hat = work + 3 * n_rows;
  device float *v = work + 4 * n_rows;
  device float *s = work + 5 * n_rows;
  device float *s_hat = work + 6 * n_rows;
  device float *t = work + 7 * n_rows;
  device float *ax = work + 8 * n_rows;
  ...
  for (int i = static_cast<int>(lane); i < n_rows;
    ...
    rr_acc += ri * ri;
    bb_acc += b[i] * b[i];
    invalid_acc += (!isfinite(ri) || !isfinite(b[i]) || !isfinite(x[i]) ||
                    !isfinite(inv_diag[i]))
                       ? 1.0f
                       : 0.0f;
  }
  ...
  for (int it = 1; it <= maxiter; ++it) {
    ...
    float p_hat_invalid = 0.0f;
    for (int i = static_cast<int>(lane); i < n_rows;
         i += static_cast<int>(k_linalg_threads)) {
      const float zi = inv_diag[i] * p[i];
      p_hat[i] = zi;
      p_hat_invalid += !isfinite(zi) ? 1.0f : 0.0f;
    }
    const float p_hat_invalid_sum =
        reduce_sum_256(p_hat_invalid, scratch, lane);
    if (lane == 0 && p_hat_invalid_sum != 0.0f) {
      shared_status = -3;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    if (shared_status < 0) {
    ...
    csr_spmv_f32(data, indices, indptr, p_hat, v, n_rows, lane);
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    for (int i = static_cast<int>(lane); i < n_rows;
      ...
      const float xi = x[i] + shared_alpha * p_hat[i];
      s[i] = si;
      x[i] = xi;
      s_acc += si * si;
    ...
    float s_hat_invalid = 0.0f;
    for (int i = static_cast<int>(lane); i < n_rows;
         i += static_cast<int>(k_linalg_threads)) {
      const float zi = inv_diag[i] * s[i];
      s_hat[i] = zi;
      s_hat_invalid += !isfinite(zi) ? 1.0f : 0.0f;
    }
    const float s_hat_invalid_sum =
        reduce_sum_256(s_hat_invalid, scratch, lane);
    if (lane == 0 && s_hat_invalid_sum != 0.0f) {
      shared_status = -3;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    if (shared_status < 0) {
    ...
    csr_spmv_f32(data, indices, indptr, s_hat, t, n_rows, lane);
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    for (int i = static_cast<int>(lane); i < n_rows;
         i += static_cast<int>(k_linalg_threads)) {
      const float xi = x[i] + shared_omega * s_hat[i];
...
template [[host_name("csr_bicgstab_jacobi_float32_int32")]] [[kernel]] void
csr_bicgstab_jacobi_kernel<int>(device const float *, device const int *,
...
template [[host_name("csr_bicgstab_jacobi_float32_int64")]] [[kernel]] void
csr_bicgstab_jacobi_kernel<long>(device const float *, device const long *,
