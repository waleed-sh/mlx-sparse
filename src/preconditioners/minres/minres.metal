...
template <typename I>
[[kernel]] void csr_minres_jacobi_kernel(
    ...
    constant float &atol [[buffer(15)]], constant float &shift [[buffer(16)]],
  ...
  if (lane == 0) {
    ...
    shared_cs = -1.0f;
    shared_sn = 0.0f;
    if (invalid0 != 0.0f || !isfinite(rr0) || !isfinite(beta0) ||
        !isfinite(shared_residual)) {
      shared_status = -3;
    } else if (nonpos0 != 0.0f) {
      shared_status = -2;
  ...
  for (int it = 1; it <= maxiter; ++it) {
    ...
    for (int i = static_cast<int>(lane); i < n_rows;
      ...
      const float new_y = inv_diag[i] * new_r;
      r1[i] = r2[i];
      r2[i] = new_r;
      y[i] = new_y;
      r_invalid += (!isfinite(new_r) || !isfinite(new_y)) ? 1.0f : 0.0f;
    }
...
template [[host_name("csr_minres_jacobi_float32_int32")]] [[kernel]] void
csr_minres_jacobi_kernel<int>(device const float *, device const int *,
...
template [[host_name("csr_minres_jacobi_float32_int64")]] [[kernel]] void
csr_minres_jacobi_kernel<long>(device const float *, device const long *,
