...
template <typename I>
[[kernel]] void csr_arnoldi_jacobi_kernel(
    device const float *data [[buffer(0)]],
    device const I *indices [[buffer(1)]], device const I *indptr [[buffer(2)]],
    device const float *inv_diag [[buffer(3)]],
    device const float *v0 [[buffer(4)]], device float *h [[buffer(5)]],
    device float *basis [[buffer(6)]], device int *actual [[buffer(7)]],
    device float *work [[buffer(8)]], constant int &n_rows [[buffer(9)]],
    constant int &n_cols [[buffer(10)]], constant int &k [[buffer(11)]],
    uint lane [[thread_index_in_threadgroup]]) {
  (void)n_cols;
  const int cols = k + 1;
  threadgroup float scratch[256];
  threadgroup float shared_scalar = 0.0f;
  threadgroup int shared_done = 0;
  threadgroup int shared_used = 0;
  ...
  for (int row = static_cast<int>(lane); row < n_rows;
    ...
    invalid_local +=
        (!isfinite(v0[row]) || !isfinite(inv_diag[row])) ? 1.0f : 0.0f;
  }
  const float invalid = reduce_sum_256(invalid_local, scratch, lane);
  const float norm0 =
      sqrt(max(reduce_sum_256(norm_local, scratch, lane), 0.0f));
  if (lane == 0) {
    shared_done = (invalid != 0.0f || !isfinite(norm0)) ? 1 : 0;
    shared_used = 0;
  }
  ...
  for (int j = 0; j < k; ++j) {
    ...
    float invalid_next_local = 0.0f;
    for (int row = static_cast<int>(lane); row < n_rows;
         row += static_cast<int>(k_linalg_threads)) {
      h_next_local += work[row] * work[row];
      invalid_next_local += !isfinite(work[row]) ? 1.0f : 0.0f;
    }
    const float invalid_next =
        reduce_sum_256(invalid_next_local, scratch, lane);
    const float h_next =
    ...
    if (lane == 0) {
      ...
      shared_used = j + 1;
      if (invalid_next != 0.0f || !isfinite(h_next) ||
          h_next <= 1.1920928955078125e-7f) {
        shared_done = 1;
      }
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
...
template [[host_name("csr_arnoldi_jacobi_float32_int32")]] [[kernel]] void
csr_arnoldi_jacobi_kernel<int>(device const float *, device const int *,
...
template [[host_name("csr_arnoldi_jacobi_float32_int64")]] [[kernel]] void
csr_arnoldi_jacobi_kernel<long>(device const float *, device const long *,
