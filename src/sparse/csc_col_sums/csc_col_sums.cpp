...
class CSCColSums : public mx::Primitive {
public:
  CSCColSums(mx::Stream stream, int n_rows, int n_cols)
  ...
  bool is_equivalent(const mx::Primitive &other) const override {
    const auto &rhs = static_cast<const CSCColSums &>(other);
...
void csc_col_sums_cpu_impl(const mx::array &data, const mx::array &indptr,
...
void CSCColSums::eval_cpu(const std::vector<mx::array> &inputs,
                          std::vector<mx::array> &outputs) {
  auto &data = inputs[0];
  auto &indptr = inputs[2];

#define DISPATCH_CSC_COL_SUMS(DTYPE, TYPE)                                     \
  if (data.dtype() == DTYPE) {                                                 \
    if (indptr.dtype() == mx::int32) {                                         \
      csc_col_sums_cpu_impl<TYPE, int32_t>(data, indptr, outputs[0], n_cols_,  \
                                           stream());                          \
    } else {                                                                   \
      csc_col_sums_cpu_impl<TYPE, int64_t>(data, indptr, outputs[0], n_cols_,  \
                                           stream());                          \
    }                                                                          \
    return;                                                                    \
  }

  DISPATCH_CSC_COL_SUMS(mx::float32, float)
  DISPATCH_CSC_COL_SUMS(mx::float16, mx::float16_t)
  DISPATCH_CSC_COL_SUMS(mx::bfloat16, mx::bfloat16_t)
  DISPATCH_CSC_COL_SUMS(mx::complex64, mx::complex64_t)
...
#ifdef _METAL_
void CSCColSums::eval_gpu(const std::vector<mx::array> &inputs,
                          std::vector<mx::array> &outputs) {
  auto &data = inputs[0];
  auto &indptr = inputs[2];
  auto &out = outputs[0];
  ...
  auto kernel_name = sparse_kernel_name(
      use_vector_kernel ? "csc_col_sums_vector" : "csc_col_sums", data.dtype(),
      indptr.dtype());
...
#else
void CSCColSums::eval_gpu(const std::vector<mx::array> &,
                          std::vector<mx::array> &) {
  throw std::runtime_error(
      "csc_col_sums has no GPU implementation in this build.");
}
#endif

mx::array csc_col_sums(const mx::array &data, const mx::array &indices,
  ...
  return mx::array(mx::Shape{n_cols}, data.dtype(),
                   std::make_shared<CSCColSums>(stream, n_rows, n_cols),
