...
class COORowNorms : public mx::Primitive {
public:
  COORowNorms(mx::Stream stream, int n_rows, int n_cols)
  ...
  const char *name() const override { return "COORowNorms"; }

  bool is_equivalent(const mx::Primitive &other) const override {
    const auto &rhs = static_cast<const COORowNorms &>(other);
...
void coo_row_norms_cpu_impl(const mx::array &data, const mx::array &row,
  ...
  encoder.dispatch([data = mx::array::unsafe_weak_copy(data),
    ...
    auto run_serial = [&]() {
      ...
      for (size_t p = 0; p < data.size(); ++p) {
        accum[static_cast<size_t>(row_ptr[p])] += norm_square<T>(data_ptr[p]);
      }
      for (int r = 0; r < n_rows; ++r) {
        out_ptr[r] =
            static_cast<float>(std::sqrt(accum[static_cast<size_t>(r)]));
      }
    };
    ...
    auto reduce_rows = [&](CpuRange range) {
      for (int row_idx = range.begin; row_idx < range.end; ++row_idx) {
        ...
        for (size_t worker = 0; worker < n_partitions; ++worker) {
          total += partial[worker * stride + row_idx];
        }
        out_ptr[row_idx] = static_cast<float>(std::sqrt(total));
      }
    };
...
void COORowNorms::eval_cpu(const std::vector<mx::array> &inputs,
                           std::vector<mx::array> &outputs) {
  auto &data = inputs[0];
  auto &row = inputs[1];

#define DISPATCH_COO_ROW_NORMS(DTYPE, TYPE)                                    \
  if (data.dtype() == DTYPE) {                                                 \
    if (row.dtype() == mx::int32) {                                            \
      coo_row_norms_cpu_impl<TYPE, int32_t>(data, row, outputs[0], n_rows_,    \
                                            stream());                         \
    } else {                                                                   \
      coo_row_norms_cpu_impl<TYPE, int64_t>(data, row, outputs[0], n_rows_,    \
                                            stream());                         \
    }                                                                          \
    return;                                                                    \
  }

  DISPATCH_COO_ROW_NORMS(mx::float32, float)
  DISPATCH_COO_ROW_NORMS(mx::float16, mx::float16_t)
  DISPATCH_COO_ROW_NORMS(mx::bfloat16, mx::bfloat16_t)
  DISPATCH_COO_ROW_NORMS(mx::complex64, mx::complex64_t)
#undef DISPATCH_COO_ROW_NORMS

  throw std::runtime_error("coo_row_norms unsupported value dtype.");
}

#ifdef _METAL_
void COORowNorms::eval_gpu(const std::vector<mx::array> &inputs,
                           std::vector<mx::array> &outputs) {
  auto &data = inputs[0];
  auto &row = inputs[1];
  auto &out = outputs[0];
  ...
  auto *zero_kernel = device.get_kernel("coo_row_norms_zero_float32", lib);
  ...
  auto kernel_name =
      sparse_kernel_name("coo_row_norms_atomic", data.dtype(), row.dtype());
  ...
  auto *sqrt_kernel = device.get_kernel("coo_row_norms_sqrt_float32", lib);
...
#else
void COORowNorms::eval_gpu(const std::vector<mx::array> &,
                           std::vector<mx::array> &) {
  throw std::runtime_error(
      "coo_row_norms has no GPU implementation in this build.");
}
#endif

mx::array coo_row_norms(const mx::array &data, const mx::array &row,
                        const mx::array &col, int n_rows, int n_cols,
                        mx::StreamOrDevice s) {
