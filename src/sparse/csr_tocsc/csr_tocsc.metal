...
[[kernel]] void csr_tocsc_zero_offsets(device int *offsets [[buffer(0)]],
...
template <typename I>
[[kernel]] void csr_tocsc_prefix_kernel(device const int *counts [[buffer(0)]],
...
[[kernel]] void csr_tocsc_fill_kernel(
  ...
  for (I p = indptr[row]; p < indptr[row + 1]; ++p) {
    ...
    out_data[dst] = data[p];
    out_indices[dst] = static_cast<I>(row);
  }
}
...
INSTANTIATE_CSR_TOCSC_FILL(float16_int32, half, int);
INSTANTIATE_CSR_TOCSC_FILL(float16_int64, half, long);
INSTANTIATE_CSR_TOCSC_FILL(bfloat16_int32, bfloat16_t, int);
INSTANTIATE_CSR_TOCSC_FILL(bfloat16_int64, bfloat16_t, long);
INSTANTIATE_CSR_TOCSC_FILL(complex64_int32, complex64_t, int);
INSTANTIATE_CSR_TOCSC_FILL(complex64_int64, complex64_t, long);

#undef INSTANTIATE_CSR_TOCSC_FILL
