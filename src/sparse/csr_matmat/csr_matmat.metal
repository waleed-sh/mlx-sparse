...
template <typename T> inline bool csr_matmat_nonzero(T value) {
  return !(value == T(0));
}
...
[[kernel]] void csr_matmat_symbolic_kernel(
  ...
  for (LhsI lhs_pos = lhs_indptr[row]; lhs_pos < lhs_indptr[row + 1];
    ...
    for (RhsI rhs_pos = rhs_indptr[rhs_row];
      ...
      if (csr_matmat_first_candidate<LhsI, RhsI>(
...
[[kernel]] void csr_matmat_prune_counts_kernel(
  ...
  for (I p = indptr[row]; p < indptr[row + 1]; ++p) {
    if (csr_matmat_nonzero<T>(data[p])) {
      count += I(1);
    }
  }
  counts[row] = count;
...
INSTANTIATE_CSR_MATMAT_SYMBOLIC(int32, int32, int32, int, int, int);
INSTANTIATE_CSR_MATMAT_SYMBOLIC(int32, int32, int64, int, int, long);
INSTANTIATE_CSR_MATMAT_SYMBOLIC(int32, int64, int64, int, long, long);
INSTANTIATE_CSR_MATMAT_SYMBOLIC(int64, int32, int64, long, int, long);
INSTANTIATE_CSR_MATMAT_SYMBOLIC(int64, int64, int64, long, long, long);

#undef INSTANTIATE_CSR_MATMAT_SYMBOLIC
...
#define INSTANTIATE_CSR_MATMAT_NUMERIC_INDEXES(VNAME, T)                       \
  ...
  INSTANTIATE_CSR_MATMAT_NUMERIC(VNAME, int32, int64, int64, T, int, long,     \
                                 long);                                        \
  INSTANTIATE_CSR_MATMAT_NUMERIC(VNAME, int64, int32, int64, T, long, int,     \
                                 long);                                        \
  INSTANTIATE_CSR_MATMAT_NUMERIC(VNAME, int64, int64, int64, T, long, long,    \
                                 long)

INSTANTIATE_CSR_MATMAT_NUMERIC_INDEXES(float32, float);
INSTANTIATE_CSR_MATMAT_NUMERIC_INDEXES(float16, half);
INSTANTIATE_CSR_MATMAT_NUMERIC_INDEXES(bfloat16, bfloat16_t);
INSTANTIATE_CSR_MATMAT_NUMERIC_INDEXES(complex64, complex64_t);

#undef INSTANTIATE_CSR_MATMAT_NUMERIC_INDEXES
#undef INSTANTIATE_CSR_MATMAT_NUMERIC

#define INSTANTIATE_CSR_MATMAT_PRUNE_COUNTS(NAME, T, I)                        \
  template [[host_name("csr_matmat_prune_counts_" #NAME)]] [[kernel]] void     \
  csr_matmat_prune_counts_kernel<T, I>(device const T *, device const I *,     \
                                       device I *, constant int &, uint)

INSTANTIATE_CSR_MATMAT_PRUNE_COUNTS(float32_int32, float, int);
INSTANTIATE_CSR_MATMAT_PRUNE_COUNTS(float32_int64, float, long);
INSTANTIATE_CSR_MATMAT_PRUNE_COUNTS(float16_int32, half, int);
INSTANTIATE_CSR_MATMAT_PRUNE_COUNTS(float16_int64, half, long);
INSTANTIATE_CSR_MATMAT_PRUNE_COUNTS(bfloat16_int32, bfloat16_t, int);
INSTANTIATE_CSR_MATMAT_PRUNE_COUNTS(bfloat16_int64, bfloat16_t, long);
INSTANTIATE_CSR_MATMAT_PRUNE_COUNTS(complex64_int32, complex64_t, int);
INSTANTIATE_CSR_MATMAT_PRUNE_COUNTS(complex64_int64, complex64_t, long);

#undef INSTANTIATE_CSR_MATMAT_PRUNE_COUNTS
...
INSTANTIATE_CSR_MATMAT_PRUNE_FILL(float16_int32, half, int);
INSTANTIATE_CSR_MATMAT_PRUNE_FILL(float16_int64, half, long);
INSTANTIATE_CSR_MATMAT_PRUNE_FILL(bfloat16_int32, bfloat16_t, int);
INSTANTIATE_CSR_MATMAT_PRUNE_FILL(bfloat16_int64, bfloat16_t, long);
INSTANTIATE_CSR_MATMAT_PRUNE_FILL(complex64_int32, complex64_t, int);
INSTANTIATE_CSR_MATMAT_PRUNE_FILL(complex64_int64, complex64_t, long);

#undef INSTANTIATE_CSR_MATMAT_PRUNE_FILL
