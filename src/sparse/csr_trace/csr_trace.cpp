...
std::vector<mx::array> CSRTrace::jvp(const std::vector<mx::array> &primals,
                                     const std::vector<mx::array> &tangents,
                                     const std::vector<int> &argnums) {
  std::vector<mx::array> terms;
  terms.reserve(argnums.size());
  for (size_t i = 0; i < argnums.size(); ++i) {
    require_sparse_value_autodiff_arg(argnums[i], "CSRTrace", "JVP");
    terms.push_back(csr_trace(tangents[i], primals[1], primals[2], n_rows_,
                              n_cols_, stream()));
  }
  if (terms.empty()) {
    throw std::runtime_error("CSRTrace JVP requires at least one tangent.");
  }
  auto result = terms[0];
...
std::vector<mx::array> CSRTrace::vjp(const std::vector<mx::array> &primals,
                                     const std::vector<mx::array> &cotangents,
                                     const std::vector<int> &argnums,
  ...
  for (int argnum : argnums) {
    require_sparse_value_autodiff_arg(argnum, "CSRTrace", "VJP");
