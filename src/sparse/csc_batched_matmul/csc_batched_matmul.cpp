...
std::pair<std::vector<mx::array>, std::vector<int>>
CSCBatchedMatMul::vmap(const std::vector<mx::array> &inputs,
                       const std::vector<int> &axes) {
  require_vmap_arity(inputs, axes, 4, "CSCBatchedMatMul");
  require_fixed_sparse_vmap_axes(axes, 3, "CSCBatchedMatMul");

  auto rhs = dense_rhs_with_vmap_axis_front(inputs[3], axes[3], stream(),
                                            "CSCBatchedMatMul");
  require_vmap_rhs_rank(rhs, 4, "CSCBatchedMatMul");
  require_vmap_rhs_dim(rhs, 1, batch_size_, "dense RHS batch dimension",
                       "CSCBatchedMatMul");
  require_vmap_rhs_sparse_dim(rhs, 2, n_cols_, "CSCBatchedMatMul");
  require_vmap_rhs_dim(rhs, 3, rhs_cols_, "dense RHS column dimension",
                       "CSCBatchedMatMul");

  const int outer_batch = rhs.shape(0);
  const int flat_batch =
      checked_vmap_batch_product(outer_batch, batch_size_, "CSCBatchedMatMul");
  auto flat_rhs =
      mx::reshape(rhs, mx::Shape{flat_batch, n_cols_, rhs_cols_}, stream());
  auto flat_out = csc_batched_matmul(inputs[0], inputs[1], inputs[2], flat_rhs,
                                     n_rows_, n_cols_, stream());
  auto out = mx::reshape(
