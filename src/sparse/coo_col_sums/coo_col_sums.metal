...
[[kernel]] void coo_col_sums_zero_complex64(device complex64_t *out
                                            [[buffer(0)]],
                                            constant int &n_cols [[buffer(1)]],
                                            uint col
                                            [[thread_position_in_grid]]) {
...
template <typename I>
[[kernel]] void coo_col_sums_atomic_complex64_kernel(
  ...
  if (c >= 0 && c < n_cols) {
    ...
    atomic_fetch_add_explicit(&atomic_out[2 * c + 1], data[p].imag,
                              memory_order_relaxed);
  }
}
