...
class CSRRowNorms : public mx::Primitive {
public:
  CSRRowNorms(mx::Stream stream, int n_rows, int n_cols)
  ...
  const char *name() const override { return "CSRRowNorms"; }

  bool is_equivalent(const mx::Primitive &other) const override {
    const auto &rhs = static_cast<const CSRRowNorms &>(other);
...
void csr_row_norms_cpu_impl(const mx::array &data, const mx::array &indptr,
  ...
  encoder.dispatch([data = mx::array::unsafe_weak_copy(data),
    ...
    auto compute_rows = [&](CpuRange range) {
      for (int row = range.begin; row < range.end; ++row) {
        ...
        out_ptr[row] = static_cast<float>(std::sqrt(acc));
      }
    };
...
void CSRRowNorms::eval_cpu(const std::vector<mx::array> &inputs,
                           std::vector<mx::array> &outputs) {
  auto &data = inputs[0];
  auto &indptr = inputs[1];

#define DISPATCH_CSR_ROW_NORMS(DTYPE, TYPE)                                    \
  if (data.dtype() == DTYPE) {                                                 \
    if (indptr.dtype() == mx::int32) {                                         \
      csr_row_norms_cpu_impl<TYPE, int32_t>(data, indptr, outputs[0], n_rows_, \
                                            stream());                         \
    } else {                                                                   \
      csr_row_norms_cpu_impl<TYPE, int64_t>(data, indptr, outputs[0], n_rows_, \
                                            stream());                         \
    }                                                                          \
    return;                                                                    \
  }

  DISPATCH_CSR_ROW_NORMS(mx::float32, float)
  DISPATCH_CSR_ROW_NORMS(mx::float16, mx::float16_t)
  DISPATCH_CSR_ROW_NORMS(mx::bfloat16, mx::bfloat16_t)
  DISPATCH_CSR_ROW_NORMS(mx::complex64, mx::complex64_t)
#undef DISPATCH_CSR_ROW_NORMS

  throw std::runtime_error("csr_row_norms unsupported value dtype.");
}

#ifdef _METAL_
void CSRRowNorms::eval_gpu(const std::vector<mx::array> &inputs,
                           std::vector<mx::array> &outputs) {
  auto &data = inputs[0];
  auto &indptr = inputs[1];
  auto &out = outputs[0];
  ...
  auto kernel_name = sparse_kernel_name(
      use_vector_kernel ? "csr_row_norms_vector" : "csr_row_norms",
...
#else
void CSRRowNorms::eval_gpu(const std::vector<mx::array> &,
                           std::vector<mx::array> &) {
  throw std::runtime_error(
      "csr_row_norms has no GPU implementation in this build.");
}
#endif

mx::array csr_row_norms(const mx::array &data, const mx::array &indices,
  ...
  return mx::array(mx::Shape{n_rows}, mx::float32,
                   std::make_shared<CSRRowNorms>(stream, n_rows, n_cols),
