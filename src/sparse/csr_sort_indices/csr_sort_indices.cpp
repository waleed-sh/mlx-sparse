...
class CSRSortIndices : public mx::Primitive {
public:
  explicit CSRSortIndices(mx::Stream stream) : Primitive(stream) {}
  ...
  const char *name() const override { return "CSRSortIndices"; }

  bool is_equivalent(const mx::Primitive &) const override { return true; }
};

template <typename T, typename I>
void csr_sort_indices_cpu_impl(const mx::array &data, const mx::array &indices,
  ...
  encoder.dispatch(
      [data = mx::array::unsafe_weak_copy(data),
       ...
       out_indptr = mx::array::unsafe_weak_copy(out_indptr)]() mutable {
        ...
        const auto n_rows = static_cast<int>(indptr.size()) - 1;
        auto sort_rows = [&](CpuRange range) {
          std::vector<size_t> order;
          for (int row = range.begin; row < range.end; ++row) {
            const auto start = static_cast<size_t>(indptr_ptr[row]);
            const auto end = static_cast<size_t>(indptr_ptr[row + 1]);
        ...
        parallel_for_cpu_ranges(ranges, sort_rows);
      });
}

} // namespace

void CSRSortIndices::eval_cpu(const std::vector<mx::array> &inputs,
  ...
  if (indices.dtype() != mx::int32 && indices.dtype() != mx::int64) {
    throw std::runtime_error(
        "csr_sort_indices requires int32 or int64 indices.");
  }

#define DISPATCH_CSR_SORT_VALUE(DTYPE, TYPE)                                   \
  if (data.dtype() == DTYPE) {                                                 \
    if (indices.dtype() == mx::int32) {                                        \
      csr_sort_indices_cpu_impl<TYPE, int32_t>(data, indices, indptr,          \
                                               outputs[0], outputs[1],         \
                                               outputs[2], stream());          \
    } else {                                                                   \
      csr_sort_indices_cpu_impl<TYPE, int64_t>(data, indices, indptr,          \
                                               outputs[0], outputs[1],         \
                                               outputs[2], stream());          \
    }                                                                          \
    return;                                                                    \
  ...
  DISPATCH_CSR_SORT_VALUE(mx::float32, float)
  DISPATCH_CSR_SORT_VALUE(mx::float16, mx::float16_t)
  DISPATCH_CSR_SORT_VALUE(mx::bfloat16, mx::bfloat16_t)
  DISPATCH_CSR_SORT_VALUE(mx::complex64, mx::complex64_t)
#undef DISPATCH_CSR_SORT_VALUE

  throw std::runtime_error("csr_sort_indices unsupported value dtype.");
}

#ifdef _METAL_
void CSRSortIndices::eval_gpu(const std::vector<mx::array> &inputs,
                              std::vector<mx::array> &outputs) {
  auto &data = inputs[0];
  auto &indices = inputs[1];
  auto &indptr = inputs[2];
  ...
  auto kernel_name =
      sparse_kernel_name("csr_sort_indices", data.dtype(), indices.dtype());
...
#else
void CSRSortIndices::eval_gpu(const std::vector<mx::array> &,
                              std::vector<mx::array> &) {
  throw std::runtime_error(
      "csr_sort_indices has no GPU implementation in this build.");
}
#endif

std::tuple<mx::array, mx::array, mx::array>
csr_sort_indices(const mx::array &data, const mx::array &indices,
                 const mx::array &indptr, mx::StreamOrDevice s) {
  require_rank(data, 1, "csr_sort_indices data");
  require_rank(indices, 1, "csr_sort_indices indices");
  require_rank(indptr, 1, "csr_sort_indices indptr");
  require_supported_value_dtype(data, "csr_sort_indices data");
  require_same_index_dtype(indices, indptr, "csr_sort_indices indices",
                           "csr_sort_indices indptr");
  if (indices.size() != data.size()) {
    throw std::invalid_argument(
        "csr_sort_indices data and indices must have equal length.");
  }
  ...
  auto primitive = std::make_shared<CSRSortIndices>(stream);
